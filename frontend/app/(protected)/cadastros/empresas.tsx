...
import { useEmpresas, useDeleteEmpresa } from "@/features/empresas/empresas.hooks";
...
export default function EmpresasScreen() {
  ...
  const { data: empresas, isLoading, isError, refetch } = useEmpresas({ search, ativa });
  const { mutate: deleteEmpresa } = useDeleteEmpresa();

  function handleInativar(id: string) {
    Alert.alert("Inativar empresa", "Confirmar inativação?", [
      { text: "Cancelar", style: "cancel" },
      { text: "Inativar", style: "destructive", onPress: () => deleteEmpresa(id) },
    ]);
  }
  ...
  return (
    <View className="flex-1 bg-background p-4">
      <View className="flex-row items-center justify-between mb-4">
        <Text className="text-foreground text-xl font-bold">Empresas</Text>
        <Pressable
          onPress={() => router.push("/(protected)/cadastros/empresa-nova")}
          className="bg-primary px-4 py-2 rounded-md"
        >
          <Text className="text-primary-foreground text-sm font-medium">Nova empresa</Text>
        </Pressable>
      </View>
      ...
      {isError && <Text className="text-destructive text-center">Erro ao carregar empresas.</Text>}
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View className="bg-card border border-border rounded-lg p-4 mb-3">
            ...
            {item.codigo && (
              <Text className="text-muted-foreground text-sm mb-2">{item.codigo}</Text>
            )}
            <View className="flex-row gap-3 mt-2">
              <Pressable
                onPress={() => router.push(`/(protected)/cadastros/empresa-editar?id=${item.id}`)}
              >
        ...
        ListEmptyComponent={
          !isLoading ? (
            <Text className="text-muted-foreground text-center mt-8">Nenhuma empresa encontrada.</Text>
          ) : null
        }
      />
    </View>
