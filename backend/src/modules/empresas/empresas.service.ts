...
export class EmpresasService {
  ...
  async findById(id: string) {
    ...
    if (!empresa) throw new AppError({ message: "Empresa não encontrada", statusCode: 404, errorCode: ErrorCode.NOT_FOUND });
  ...
  async create(data: CreateEmpresaInput) {
    if (data.codigo) {
      const codigoEmUso = await this.empresasRepository.findByCodigo(data.codigo);
      if (codigoEmUso) throw new AppError({ message: "Código já está em uso", statusCode: 409, errorCode: ErrorCode.CONFLICT });
    }

    return this.empresasRepository.create(data);
  }

  async update(id: string, data: UpdateEmpresaInput) {
    await this.findById(id);

    if (data.codigo) {
      const codigoEmUso = await this.empresasRepository.findByCodigo(data.codigo);
      if (codigoEmUso && codigoEmUso.id !== id) {
        throw new AppError({ message: "Código já está em uso", statusCode: 409, errorCode: ErrorCode.CONFLICT });
      }
    }
